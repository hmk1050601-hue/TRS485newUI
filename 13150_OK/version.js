(async function(global){
  // ── 版本變更記錄 ──────────────────────────────────────
  // v13150: 2026-04-07 11:10:21 | 彙整本次調整：首頁隱藏設定/實際溫度下方線條、使用設定與聯繫客服按鈕縮小置中、遙控器改名溫度設定、時間設定改底部開/關按鈕且勾叉縮小；並修正安裝快取（manifest 加版次參數、SW 不攔截 manifest）避免手機桌面仍顯示 13148。
  // v13149: 2026-04-07 12:00:00 | 首頁/頁面樣式調整：隱藏設定與實際水溫下方的分隔線；縮小並置中「使用設定」與「聯繫客服」按鈕（文字改為「使用設定」）；遙控器頁面改名為「溫度設定」；時間設定頁隱藏大開關按鈕並在面板底部新增「開」/「關」按鈕（送出 /timermode 與 /shutdown），同時將勾／叉按鈕尺寸縮小約一半。
  // v13148: 2026-04-02 18:00:00 | 修正首頁連線狀態入口誤判，已連上AP時不再反覆提示未連上
