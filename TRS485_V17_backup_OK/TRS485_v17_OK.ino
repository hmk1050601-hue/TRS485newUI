...
static bool syncBoardSchedulesFromPhone(bool usePackedMode, uint8_t packedMode);
...
void send42Write16Pair(uint16_t startAddr, uint16_t v1, uint16_t v2) {
  ...
  // 協定限制：reg19(0x0013) 不可用 pair 寫法，必須使用 packed single-word。
  if (startAddr == BOARD_MODE_ADDR) {
    Serial.println("[RS485] block pair-write to reg19(0x0013); use packed single-word");
    return;
  }
...
static void writeBoardRegs7to18And20(uint8_t maskVal, bool writeMask=true) {
  // 每個時段固定單一包：2 words（開始HH/MM + 結束HH/MM）
  const uint16_t t1Start = (uint16_t)gBoardSchedRaw[0]  | ((uint16_t)gBoardSchedRaw[1]  << 8);
  const uint16_t t1End   = (uint16_t)gBoardSchedRaw[2]  | ((uint16_t)gBoardSchedRaw[3]  << 8);
  const uint16_t t2Start = (uint16_t)gBoardSchedRaw[4]  | ((uint16_t)gBoardSchedRaw[5]  << 8);
  const uint16_t t2End   = (uint16_t)gBoardSchedRaw[6]  | ((uint16_t)gBoardSchedRaw[7]  << 8);
  const uint16_t t3Start = (uint16_t)gBoardSchedRaw[8]  | ((uint16_t)gBoardSchedRaw[9]  << 8);
  const uint16_t t3End   = (uint16_t)gBoardSchedRaw[10] | ((uint16_t)gBoardSchedRaw[11] << 8);

  send42Write16Pair(BOARD_T1_START_ADDR, t1Start, t1End);
  delay(120); yield();
  send42Write16Pair(BOARD_T2_START_ADDR, t2Start, t2End);
  delay(120); yield();
  send42Write16Pair(BOARD_T3_START_ADDR, t3Start, t3End);
  delay(120); yield();
  if (writeMask) {
...
static unsigned long gScheduleSaveQuietUntilMs = 0;

static void armScheduleSaveQuietWindow(unsigned long holdMs = 6000UL) {
  gScheduleSaveQuietUntilMs = millis() + holdMs;
}

static bool scheduleSaveQuietActive() {
  return (long)(millis() - gScheduleSaveQuietUntilMs) < 0;
}

static uint8_t snapshotBoardModeForScheduleSave() {
  if (currentMode >= BOARD_MODE_OFF && currentMode <= BOARD_MODE_TIMER) {
    return (uint8_t)currentMode;
  }
  if (gBoardSchedValid && gBoardSchedRaw[12] <= BOARD_MODE_TIMER) {
    return (uint8_t)gBoardSchedRaw[12];
  }
  return BOARD_MODE_OFF;
}
...
static bool syncBoardSchedulesFromPhone(bool usePackedMode, uint8_t packedMode){
  uint8_t prevRaw[14];
  const bool hadPrev = gBoardSchedValid;
  if (hadPrev) memcpy(prevRaw, gBoardSchedRaw, sizeof(prevRaw));
  ...
  if (hadPrev) {
    rawChanged = false;
    for (int i = 0; i < 12; i++) {
      if (prevRaw[i] != gBoardSchedRaw[i]) { rawChanged = true; break; }
    }
    if (!rawChanged && prevRaw[13] != gBoardSchedRaw[13]) rawChanged = true;
  }

  // reg7~18 與 reg20 先分段寫回設備端；reg19 模式不在儲存排程時改動
  // 需求：按下儲存時固定送出時段封包，不因 no-change 跳過。
  if (!rawChanged) {
    Serial.printf("[BoardSched] no-change, but still push reg7~18/reg20 (mask=0x%02X)\n", (unsigned)mask);
  }
  // If caller wants to perform a packed mode+mask single-word write, skip separate mask write here
  writeBoardRegs7to18And20(mask, !usePackedMode);
...
void checkSchedule(bool force = false) {
  ...
  // 排程儲存後的短暫靜默窗：避免儲存流程中/剛結束時觸發自動開關。
  if (!force && scheduleSaveQuietActive()) {
...
void handleApiSchedulePost() {
  bool anyError = false;
  const uint8_t keepMode = snapshotBoardModeForScheduleSave();


  // 儲存當下與後續數秒只做資料寫入，避免任何自動排程切模式。
  armScheduleSaveQuietWindow();

  for (int i = 0; i < MAX_SCHEDULES; i++) {
    String idx = String(i);

    bool enabled = gSchedules[i].enabled;
  ...
  saveSchedules();
  // Use packed single-word write for reg19+reg20 when saving schedules
  const bool rawChanged = syncBoardSchedulesFromPhone(true, keepMode);

  // 如果前端送了 mask 參數，使用該 mask（bit0..2 = 時段1~3）覆蓋
  uint8_t outMask = gBoardSchedRaw[13] & 0x07;
  if (server.hasArg("mask")) {
    int mv = server.arg("mask").toInt();
    outMask = (uint8_t)(mv & 0x07);
    gBoardSchedRaw[13] = outMask;
  }

  // 依現場規則：儲存排程時最後只寫 reg20（mask），不覆寫 reg19(mode)
  send42Write16(BOARD_MASK_ADDR, outMask);

  Serial.printf("[ScheduleSave] keepMode=%u, mask=0x%02X, modeWrite=skip, maskWrite=reg20_single, rawChanged=%s\n", (unsigned)keepMode, (unsigned)outMask, rawChanged ? "yes" : "no");
  currentMode = keepMode;
  gBoardSchedRaw[12] = keepMode;
  gBoardSchedValid = true;
  gBoardSchedLastMs = millis();
  armScheduleSaveQuietWindow();
...
void handleApiBoardSchedPost(){
  ...
  for (int i = 0; i < 3; i++) {
    ...
    bool enabled = gSchedules[i + 1].enabled;
    if (server.hasArg("en" + idx)) {
      String e = server.arg("en" + idx);
      enabled = (e == "1" || e == "true" || e == "on");
    }

    // 缺參數或解析失敗時，沿用既有值，避免被寫成 00:00
    int startMin = gSchedules[i + 1].startMin;
    int endMin   = gSchedules[i + 1].endMin;
    if (server.hasArg("s" + idx)) {
      int m = parseHM(server.arg("s" + idx));
      if (m >= 0) startMin = m; else anyError = true;
    }
  ...
  saveSchedules();
  // Use packed single-word write for reg19+reg20 when saving board schedules
  const bool rawChanged = syncBoardSchedulesFromPhone(true, keepMode);
  ...
  Serial.printf("[BoardSchedSave] keepMode=%u, mask=0x%02X, modeWrite=skip, maskWrite=reg20_single, rawChanged=%s\n", (unsigned)keepMode, (unsigned)outMask, rawChanged ? "yes" : "no");
  currentMode = keepMode;
  gBoardSchedRaw[12] = keepMode;
  gBoardSchedValid = true;
  gBoardSchedLastMs = millis();
