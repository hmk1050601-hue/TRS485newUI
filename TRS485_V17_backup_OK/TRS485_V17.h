...
static bool syncBoardSchedulesFromPhone(){
  uint8_t prevRaw[14];
  ...
  if (rawChanged) {
    ...
    Serial.printf("[BoardSched] no-change, skip reg7~18/reg20 write (mask=0x%02X)\n", (unsigned)mask);
  }
...
<button id="go" class="btn">返回控制首頁</button>
<small>將於 %MS% 秒後回到控制首頁。</small>
<script>
(function(){
  var sec = %MS%;
  function goHome(){
...
void handleApiSchedulePost() {
  ...
  saveSchedules();
  const bool regsWritten = syncBoardSchedulesFromPhone();

  // 儲存排程不主動改 reg19：避免 TIMER 模式下出現瞬間啟停。
  Serial.printf("[ScheduleSave] keepMode=%u, mask=0x%02X, modeWrite=skip, regsWrite=%s\n", (unsigned)keepMode, (unsigned)gBoardSchedRaw[13], regsWritten ? "yes" : "no");
...
void handleApiBoardSchedPost(){
  ...
  // 儲存排程不主動改 reg19：避免 TIMER 模式下出現瞬間啟停。
  Serial.printf("[BoardSchedSave] keepMode=%u, mask=0x%02X, modeWrite=skip, regsWrite=%s\n", (unsigned)keepMode, (unsigned)gBoardSchedRaw[13], regsWritten ? "yes" : "no");
  currentMode = keepMode;
  gBoardSchedRaw[12] = keepMode;
  gBoardSchedValid = true;
  gBoardSchedLastMs = millis();
...
void setup() {
  ...
  server.on("/timermode", HTTP_OPTIONS, [](){ addCors(); server.send(204); });
