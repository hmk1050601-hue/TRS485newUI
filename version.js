(async function(global){
  function injectGlobalScrollLock(){
    try{
      if(typeof document === 'undefined') return;

      const styleId = 'hmk-global-scroll-lock';
  ...
  injectGlobalScrollLock();
  // ── 版本變更記錄 ──────────────────────────────────────
  // v13164: 2026-04-08 15:52:38 | 自動版更與備註更新
  // v13163: 2026-04-08 15:45:41 | 自動版更與備註更新
  // v13162: 2026-04-08 14:52:47 | 首頁回跳加速：水溫/熱水器狀態加入快取預顯示；IP 模式回首頁降低 AP 探測與重複刷新，減少斷線重啟
  // v13161: 2026-04-08 14:19:33 | 自動版更與備註更新
  // v13160: 2026-04-08 13:43:40 | 排程儲存改為固定送出 3 包時段（開始+結束同包）與 1 包 reg20，並保留 mode 不覆寫
  // v13159: 2026-04-07 16:00:00 | 同步 HMK 標題圖與子頁標題更新
  // v13158: 2026-04-07 15:20:00 | 修正子頁新舊交替：統一子頁導頁帶版號，SW 回退改命中 canonical 殼頁避免舊 query 快取交錯
  // v13157: 2026-04-07 14:30:00 | 子頁面加入 version.js 載入，確保每頁都有版本檢查與快取清除
  // v13156: 2026-04-07 14:00:00 | 修正更新機制：sw.js 加版本戳觸發瀏覽器重裝 SW，fetch handler 加 no-store 確保真網路優先
  // v13155: 2026-04-07 12:50:00 | 強化版本換殼：version.js 改為清除所有 Cache Storage 並加時間戳重載，避免持續停留舊 HTML
  // v13154: 2026-04-07 12:20:00 | 永久修正快取殘留：由 version.js 依版本一次性清理 SW/Cache 並重載 HTML，避免只更新版號不更新畫面
  // v13153: 2026-04-07 11:55:00 | 新增根目錄可視化載入標記（ROOT/REMOTE HTML 13153），並強制快取升版診斷舊頁殘留問題
  // v13152: 2026-04-07 11:40:00 | 根目錄快取強制刷新：首頁導頁加版本參數，並一次性清理 SW/Cache，修正更新後仍顯示舊頁問題
  ...
  async function enforceRuntimeHtmlRefresh(){
    try{
      if(location.hostname === '192.168.4.1') return false;

      const key = 'hmk_runtime_html_refresh_done_v' + String(version);
      ...
      console.warn('runtime html refresh skipped', e);
      return false;
    }
  }

  const refreshed = await enforceRuntimeHtmlRefresh();
  if(refreshed) return;
  ...
  function isRemotePage(){
    ...
    return p.endsWith('/remote.html') || p === '/remote.html';
  }

  function applyUiTextHotfix(){
    try{
      const tile = document.querySelector('#controlBtn .tileLabel');
      if(tile) tile.textContent = '溫度設定';

      if(isRemotePage()){
        const backText = document.querySelector('#backBtn span');
        if(backText && /遙控器|時間設定/.test(String(backText.textContent || ''))){
          backText.textContent = '溫度設定';
        }

        if(/遙控器|時間設定/.test(String(document.title || ''))){
          document.title = String(document.title).replace(/遙控器|時間設定/g, '溫度設定');
        }
      }
  ...
  applyUiTextHotfix();
  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', applyUiTextHotfix, { once:true });
  }
  setTimeout(applyUiTextHotfix, 600);
  setTimeout(applyUiTextHotfix, 1600);
