...
function Get-FirstMeaningfulLine([string]$text) {
    if (-not $text) { return '' }
    $clean = $text.Replace([char]0, '')
    $clean = $clean -replace "\uFEFF", ''
    $lines = ($clean -split "`r?`n") | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne '' }
...
function Is-InvalidChangelog([string]$text) {
    $x = Get-FirstMeaningfulLine $text
    if (-not $x) { return $true }
    if ($x.Length -le 1) { return $true }
    if ($x -match '^\$\(cat\s+\.latest_commit_msg\.txt\)$') { return $true }
    if ($x -match '^(?i:initial commit|init)$') { return $true }
    return $false
}

function Get-DiffSummary() {
    try {
        $u = @(git diff --name-only --relative 2>$null)
        $s = @(git diff --cached --name-only --relative 2>$null)
        $n = @(git ls-files --others --exclude-standard 2>$null)
        $all = @($u + $s + $n) |
            ForEach-Object { $_.ToString().Trim().Replace([char]0, '') } |
            Where-Object { $_ -ne '' } |
            Select-Object -Unique
        if (-not $all -or $all.Count -eq 0) { return '' }
        $top = @($all | Select-Object -First 4)
        if ($all.Count -gt 4) {
            return '更新檔案: ' + ($top -join ', ') + " 等$($all.Count)項"
        }
        return '更新檔案: ' + ($top -join ', ')
    } catch {
        return ''
    }
}
...
if ((-not $changelog) -and [Console]::IsInputRedirected) {
...
if ((Is-InvalidChangelog $changelog) -and (Test-Path $latestCommitPath)) {
    try {
        $rawCommit = Get-Content $latestCommitPath -Raw -Encoding UTF8
        if (Is-InvalidChangelog $rawCommit) {
...
if (Is-InvalidChangelog $changelog) {
    try {
        $gitMsg = (git log -1 --pretty=%B 2>$null) -join "`n"
        $gitMsg = Normalize-Changelog $gitMsg
        if (-not (Is-InvalidChangelog $gitMsg)) {
            $changelog = $gitMsg
        }
    } catch {}
}
...
if (Is-InvalidChangelog $changelog) {
    $autoSummary = Get-DiffSummary
    if (-not (Is-InvalidChangelog $autoSummary)) {
        $changelog = $autoSummary
    }
}
...
if (Test-Path $indexPath) {
    ...
    # fallback: repair corrupted placeholders like $113160"> / $113160';
    $idx = [regex]::Replace($idx, '\$\d+">', ('<link rel="manifest" href="./manifest.json?v=' + $new + '">'))
    $idx = [regex]::Replace($idx, "\$\d+';", ("const ROOT_BUILD = '" + $new + "';"))
...
if (Test-Path $swPath) {
    ...
    # fallback: repair corrupted placeholders like $113160 ← ... / $113160';
    $sw = [regex]::Replace($sw, '\$\d+\s*←[^\r\n]*', ('// SW_VERSION=' + $new + ' ← bump-version.ps1 自動更新此行，觸發瀏覽器重新安裝 SW'))
    $sw = [regex]::Replace($sw, "\$\d+';", ("const SW_VERSION = '" + $new + "';"))
...
foreach ($subPage in @('remote.html','schedule.html','status.html','powerlog.html','user.html','setup.html')) {
    ...
    if (Test-Path $subPath) {
        ...
        # fallback: repair corrupted placeholder script refs like <script src="./$113160"></script>
        $sub = [regex]::Replace($sub, '<script\s+src="\./\$\d+"\s*></script>', ('<script src="./version.js?v=' + $new + '"></script>'))
